...
function isTriangle(a, b, c) {
  ...
  const maxIndex = [a, b, c].findIndex((el) => el === max);
  const rest = [a, b, c];
  rest.splice(maxIndex, 1);
  return (rest[0] + rest[1]) > max;
}
...
function findFirstSingleChar(str) {
  ...
  return unic.filter((i) => str.split('')
    .filter((el) => el === i).length === 1)[0];
}
