...
function timeSpanToString(startDate, endDate) {
  ...
  const mSec = newDate.getMilliseconds().toString();
  return `${newDate.toGMTString().slice(-12, -4)}.${mSec.length < 2 ? `00${mSec}` : mSec}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const Q = Math.abs(0.5 * (60 * H - 11 * M));
  return ((Q > 180 ? Q - 180 : Q) * Math.PI) / 180;
}
